...
class IQADataset(Dataset):
    def __init__(self, dataset, config, index, status):
        ...
        #print("self.mos {}".format (self.mos))
        im_names = []
        ref_names = []
        ...
        NssFeatures=[]
        for line9 in open("./data_copule/parametre_NSS.txt", "r"):
            line9 = float(line9.strip())
            #print("self.NssFeatures {}".format (line9 ))
            NssFeatures.append(line9)
        NssFeatures = np.array(NssFeatures)
        print("self.NssFeatures {}  ".format (NssFeatures.shape))
        self.patches = ()
        self.features = []
        self.label = []
        ...
        NssFeatures=[NssFeatures[i*108:(i+1)*108] for i in self.index]
        ...
        for idx in range(len(self.index)):
            ...
            if status == 'train':
                ...
                for i in range(len(patches)):
                    ...
                    self.features.append(NssFeatures[idx])
            else:
